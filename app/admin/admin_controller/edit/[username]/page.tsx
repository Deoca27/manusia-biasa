...
import { checkData, detailData, setUpdateData } from "@/app/models/admin";
...
export default function EditPage({ params }: { params: { username: string } }) {
    ...
    async function fetchData() {
        // isi nilai "setValue"
        setValue(await detailData(atob(decodeURIComponent(params.username))));
    }
    ...
    const getCheckData = async (username: string) => {
        setCheck(await checkData(username));
    };
    ...
    const editData = async () => {
        ...
        getUsername == "" ? (username = refUsername.current?.value) : (username = getUsername);
        getPassword == "" ? (password = refPassword.current?.value) : (password = getPassword);
                ...
                Object.values(getCheck)?.length == 0
                    ? [
                        await setUpdateData(
                            String(username),
                            String(password),
                            atob(decodeURIComponent(params.username))
                        ),
                        alert("Data Berhasil Diubah"),
                        (location.href = `/admin/admin_controller/edit/` + btoa(String(username))),
                    ]
                    : alert("username Sudah Pernah Tersimpan !"),
            ];
    };

    return (
        <>
        <div className="text-center font-extrabold">EDIT ADMIN</div>
            {Object.values(getValue)?.map((data: any, index: number) => (
                <div key={index} className="grid grid-cols-10 gap-4 mt-16">
                    ...
                    <Link href={"/admin/admin_controller"} className="btn btn-neutral">
                        Batal
                    </Link>
                </div>
            ))}
